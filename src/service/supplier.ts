...
export const createSupplierFn = async (data: Supplier): Promise<Supplier> => {
...
export const updateSupplierFn = async (data: Supplier): Promise<Supplier> => {
	const { data: supplier } = await api.put<Supplier>(
		`/suppliers/${data.id}`,
		data,
	);
	return supplier;
